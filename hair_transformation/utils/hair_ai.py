...
# Disable tqdm progress bars from libraries to avoid background threads during generation
...
class SkinToneAwareHairTransformation:
    ...
    def _initialize_hairstyle_models(self):
        ...
        try:
            ...
            # Prefer Replicate API (qwen/qwen-image) if a token is provided
            replicate_token = os.environ.get("REPLICATE_API_TOKEN")
            if replicate_token:
                self.use_replicate = True
                self.replicate_token = replicate_token
                self.replicate_model = "qwen/qwen-image"
                # discover a model version id via the Replicate API
                try:
                    resp = requests.get(
                        f"https://api.replicate.com/v1/models/{self.replicate_model}",
                        headers={"Authorization": f"Token {self.replicate_token}"},
                        timeout=10,
                    )
                    if resp.status_code == 200:
                        model_info = resp.json()
                        # prefer default_version or latest_version if present
                        version = None
                        if isinstance(model_info, dict):
                            version = model_info.get(
                                "default_version"
                            ) or model_info.get("latest_version")
                            if isinstance(version, dict):
                                self.replicate_version = version.get("id")
                            else:
                                self.replicate_version = None
                        if not getattr(self, "replicate_version", None):
                            # fallback: list versions endpoint
                            vresp = requests.get(
                                f"https://api.replicate.com/v1/models/{self.replicate_model}/versions",
                                headers={
                            ...
                            if vresp.status_code == 200:
                                ...
                                if (
                                    ...
                                    and "results" in versions
                                    and len(versions["results"]) > 0
                                ):
                                    self.replicate_version = versions["results"][0].get(
                                        "id"
                                    )
                        if getattr(self, "replicate_version", None):
                            print(
                                f"✅ Replicate model ready: {self.replicate_model} (version: {self.replicate_version})"
                            )
                            self.models_used.append(
                                f"{self.replicate_model} (Replicate)"
                            )
                            return
                        else:
                            print(
                                "   ⚠ Replicate model version discovery failed; falling back to local pipeline"
                            )
                except Exception as e:
                    print(f"   ⚠ Replicate initialization error: {e}")

            # If Replicate was not configured or discovery failed, disable hairstyle AI
            print(
                "   ⚠ Replicate not configured or discovery failed. Hairstyle AI disabled."
            )
    ...
    def cleanup(self):
        """Attempt to free model resources and force garbage collection to avoid lingering threads."""
        try:
            if getattr(self, "hairstyle_pipe", None) is not None:
                try:
                    # Move models to CPU if possible
                    try:
                        self.hairstyle_pipe.to("cpu")
                    except Exception:
                        pass
                finally:
                    try:
                        del self.hairstyle_pipe
                    except Exception:
                        self.hairstyle_pipe = None
                    self.hairstyle_pipe = None
    ...
    # --- Replicate helpers -------------------------------------------------
    def _replicate_upload(self, file_bytes, filename="upload.png"):
        """Upload binary to Replicate uploads endpoint, return uploaded URL."""
        token = getattr(self, "replicate_token", None)
        if not token:
            raise RuntimeError("Replicate token not set")
        try:
            ...
            print(f"   ❌ Replicate upload failed: {e}")
            return None

    def _replicate_create_prediction(self, input_dict):
        """Create a Replicate prediction for the configured model version and poll until complete."""
        token = getattr(self, "replicate_token", None)
        version = getattr(self, "replicate_version", None)
        if not token or not version:
            raise RuntimeError("Replicate not initialized with token/version")
        ...
        try:
            ...
            print(f"   ❌ Replicate prediction error: {e}")
            return None
    ...
    def _get_head_hair_mask(self, image_np, face_features, all_masks):
        ...
        head_bottom = face_y + int(
            face_h * 0.3
        )  # Just below forehead (exclude beard area)
        ...
        contours, _ = cv2.findContours(
            all_masks, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        for contour in contours:
            ...
            is_in_head_region = (
                y < head_bottom and contour_center_y < face_y + face_h * 0.4
            )
            ...
            significant_overlap = (
                (overlap_with_head > total_contour_area * 0.3)
    ...
    def _choose_hair_class_from_logits(self, upsampled_logits, image_np):
        ...
        try:
            ...
            if cfg and getattr(cfg, "id2label", None):
                ...
                if hair_classes:
                    ...
                    print(
                        f"   🎯 Selected hair class: {hair_classes[0][1]} (priority {hair_classes[0][2]})"
                    )
        ...
        print(
            f"   🎯 Fallback selected class {best_idx} with upper overlap: {best_upper_overlap}"
        )
        return int(best_idx)
    ...
    def analyze_skin_tone(self, image, face_features):
        ...
        try:
            ...
            dominant_lab = cv2.cvtColor(
                np.uint8([[dominant_color]]), cv2.COLOR_RGB2LAB
            )[0][0]
            ...
            print(
                f"   ✅ Skin tone: {skin_tone} ({tone_category}), {warmth} undertones"
            )
    ...
    def get_balanced_diverse_styles(
        self, skin_analysis, face_shape, current_hair_analysis
    ):
        ...
        base_styles = expanded_styles.get(
            ethnicity, expanded_styles["East Asian/Caucasian"]
        )
        ...
        filtered_long = [
            ...
            if self.is_style_different(style, current_length, current_texture)
        ]
    ...
    def is_style_different(self, style, current_length, current_texture):
        ...
        if current_length in ["long", "very long"] and any(
            word in style_lower for word in ["short", "crop", "pixie", "buzz", "fade"]
        ):
            return True

        # If current is short, any long style is different
        if current_length in ["short", "medium"] and any(
            word in style_lower
            for word in ["long", "waist", "butt", "rapunzel", "extra long"]
        ):
            return True

        # Different texture patterns
        current_texture_lower = current_texture.lower()
        if current_texture_lower in ["straight", "wavy"] and any(
            word in style_lower for word in ["curly", "afro", "braid", "twist", "dread"]
        ):
            return True

        if current_texture_lower in ["curly", "very curly"] and any(
            word in style_lower for word in ["straight", "sleek", "blunt", "smooth"]
        ):
            return True
    ...
    def get_hair_color_recommendations(self, skin_analysis):
        ...
        tone_recs = color_recommendations.get(
            skin_tone, color_recommendations["Medium"]
        )
    ...
    def enhanced_hair_segmentation(self, image_path):
        ...
        try:
            ...
            hair_class_idx = self._choose_hair_class_from_logits(
                upsampled_logits, image_np
            )
            ...
            head_hair_mask = self._get_head_hair_mask(
                image_np, face_features, all_hair_mask
            )
            ...
            contours, _ = cv2.findContours(
                hair_mask_clean, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
            ...
            if hair_contours:
                ...
                complexity = (
                    (hair_perimeter**2) / (4 * np.pi * hair_area)
                    if hair_perimeter > 0
                    else 1.0
                )
            ...
            print(
                f"   ✅ HEAD HAIR detection: {hair_pixels} pixels ({hair_coverage:.1f}%)"
            )
            print(
                f"   📊 Hair analysis: {hair_length} {hair_type} hair (class {hair_class_idx})"
            )
            ...
            return self.fallback_segmentation(
                image_path if not isinstance(image_path, Image.Image) else image_path
            )
    ...
    def detect_face_comprehensive(self, image):
        ...
        try:
            ...
            return (
                face_features,
                gray[face_y : face_y + face_h, face_x : face_x + face_w],
    ...
    def create_texture_preserving_prompt(
        self, hairstyle, skin_analysis, face_shape, texture_features
    ):
        ...
        if texture_features and texture_features["texture_complexity"] > 25:
            texture_keywords = (
                "natural hair texture, realistic strands, detailed texture, "
            )
        ...
        if skin_tone in ["Dark", "Brown"]:
            prompt += (
                f", on person with {skin_tone.lower()} skin tone, natural hair colors"
            )
        elif skin_tone in ["Fair", "Light"]:
            prompt += (
                f", on person with {skin_tone.lower()} skin tone, natural hair tones"
            )

        return prompt

    def texture_preserving_transformation(
        self,
        original_image,
        hair_mask,
        face_features,
        ...
        # If hairstyle AI is disabled, or neither Replicate nor a local pipeline is available, use basic fallback
        if not self.use_hairstyle_ai or (
            not getattr(self, "use_replicate", False) and self.hairstyle_pipe is None
        ):
            return self.basic_ethnicity_aware_transformation(
        ...
        try:
            ...
            # create prompt
            prompt = self.create_texture_preserving_prompt(
                hairstyle, skin_analysis, face_features["shape"], texture_features
            )
            ...
            # run inpainting via Replicate if configured
            edited_resized = None
            if getattr(self, "use_replicate", False) and getattr(
                self, "replicate_version", None
            ):
                try:
                    # upload images to Replicate
                    buf_img = BytesIO()
                    init_image_resized.save(buf_img, format="PNG")
                    buf_img.seek(0)
                    img_url = self._replicate_upload(buf_img, filename="image.png")

                    buf_mask = BytesIO()
                    mask_image_resized.save(buf_mask, format="PNG")
                    buf_mask.seek(0)
                    mask_url = self._replicate_upload(buf_mask, filename="mask.png")

                    if not img_url or not mask_url:
                        raise RuntimeError("Replicate upload failed")
                    ...
                    input_payload = {
                        ...
                        # model-specific keys may vary; keep generic
                    }

                    pred = self._replicate_create_prediction(input_payload)
                    if pred and pred.get("status") == "succeeded":
                        outputs = pred.get("output") or pred.get("result") or []
                        # outputs may be list of URLs or image data
                        out_url = None
                        ...
                        if out_url:
                            ...
                            r.raise_for_status()
                            edited_resized = Image.open(BytesIO(r.content)).convert(
                                "RGB"
                            )
                            # ensure size matches target
                            if edited_resized.size != target_size:
                                edited_resized = edited_resized.resize(
            ...
            # If Replicate not used or failed, fall back to a basic non-AI transformation
            if edited_resized is None:
                print(
                    "   ⚠ No AI transformation available; using basic ethnicity-aware fallback"
                )
                return self.basic_ethnicity_aware_transformation(
                    original_image, hair_mask, hairstyle, skin_analysis
                )
            ...
            if edited_np.shape != orig_np.shape:
                print(
                    f"   ⚠️ Shape mismatch - edited: {edited_np.shape}, original: {orig_np.shape}"
                )
            ...
            if mask_np.shape[:2] != orig_np.shape[:2]:
                print(
                    f"   ⚠️ Mask shape mismatch - mask: {mask_np.shape}, original: {orig_np.shape}"
                )
    ...
    def basic_ethnicity_aware_transformation(
        self, original_image, hair_mask, hairstyle, skin_analysis
    ):
        ...
        try:
            ...
            for channel in range(3):
                result[hair_region, channel] = np.clip(
                    result[hair_region, channel] + texture[hair_region], 0, 255
                ).astype(np.uint8)

            # Return the transformed image without on-image annotations
    ...
    def full_balanced_transformation_pipeline(
        self, image_path, use_ai=True, progress_callback=None
    ):
        """Complete pipeline with balanced long/short style recommendations"""
        try:
            if progress_callback:
                try:
                    progress_callback(5, "Starting hair analysis")
                except Exception:
                    pass
            ...
            if progress_callback:
                try:
                    progress_callback(20, "Segmentation complete")
                except Exception:
                    pass

            print("🔍 Step 2: Detailed Face Analysis...")
            face_features, face_region, face_vis = self.detect_face_comprehensive(
                original_image
            )

            if progress_callback:
                try:
                    progress_callback(35, "Face analysis complete")
                except Exception:
                    pass
            ...
            if progress_callback:
                try:
                    progress_callback(50, "Skin tone analysis complete")
                except Exception:
                    pass

            print("🔍 Step 4: Natural Hair Texture Analysis...")
            texture_features = self.extract_hair_texture_features(
                original_image, hair_mask
            )

            if progress_callback:
                try:
                    progress_callback(60, "Texture analysis complete")
                except Exception:
                    pass
            ...
            if progress_callback:
                try:
                    progress_callback(70, "Color recommendations ready")
                except Exception:
                    pass
            ...
            for i, hairstyle in enumerate(style_recommendations[:4]):
                ...
                print(
                    f"   🎯 Generating {style_type} {skin_analysis['ethnicity_likely']} style {i+1}/4: {hairstyle}"
                )
                    ...
                    transformation_details.append(
                        {
                            ...
                            "skin_tone": skin_analysis["skin_tone"],
                            "method": (
                                "Texture-Preserved AI"
                                if use_ai and self.use_hairstyle_ai
                                else "Basic"
                            ),
                            "texture_preserved": texture_features is not None,
                        }
                    )
                    # update progress per generated style
                    if progress_callback:
                        try:
                            pct = 75 + int(
                                ((i + 1) / min(4, len(style_recommendations[:4]))) * 20
                            )
                            progress_callback(pct, f"Generated {i+1}/4 transformations")
                        except Exception:
                            pass
                else:
            ...
            # attempt to cleanup heavy resources (pipelines) to avoid lingering background threads
            try:
                self.cleanup()
            except Exception:
                pass
...
class DjangoHairTransformation:
    ...
    def process_image(self, image_path, session_id):
        """
        Process image and return results in Django-compatible format
        """
        try:
            # Run the transformation pipeline
            # Define a callback to update the DB progress if session_id provided
            def progress_cb(percent, message):
                try:
                    from ..models import HairTransformation

                    ht = HairTransformation.objects.filter(
                        session_id=session_id
                    ).first()
                    if ht:
                        ht.progress = int(percent or 0)
                        ht.processing_step = message or ""
                        ht.save(update_fields=["progress", "processing_step"])
                except Exception:
                    pass

            results = self.transformer.full_balanced_transformation_pipeline(
                image_path, use_ai=True, progress_callback=progress_cb
            )
            ...
            django_results = {
                ...
                "images": {
                    ...
                    "segmentation_mask": results["segmentation_results"].get(
                        "hair_mask"
                    ),
                    "face_analysis": results.get(
                        "face_vis", results["segmentation_results"]["visualization"]
                    ),
                    "transformations": [],
                },
            }
            ...
            for title, img in results["results"]:
                if "Long:" in title or "Short:" in title:
                    django_results["images"]["transformations"].append(
                        {
                            "title": title,
                            "image": img,
            ...
            print(f"Error in Django processing: {e}")
            return None

    def pil_to_django_file(self, pil_image, filename):
        """
